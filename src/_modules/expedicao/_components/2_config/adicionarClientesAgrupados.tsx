...
import { Users, Plus, Trash2, XIcon } from "lucide-react"
...
export default function AdicionarClientesAgrupados() {
  const { agruparClientes, setAgruparClientes } = useConfigGroupsImpressao()
  const [groups, setGroups] = useState<Group[]>(agruparClientes || [])
  const [newGroupName, setNewGroupName] = useState("")

  useEffect(() => {
    setGroups(agruparClientes || [])
  }, [agruparClientes])
  ...
  function handleAddGroup() {
    if (newGroupName.trim() !== "") {
      ...
      setAgruparClientes(updatedGroups)
      setNewGroupName("")
    }
  }
        ...
        <AccordionItem value="item-1">
          <AccordionTrigger className="text-lg font-medium">
            <div className="flex items-center gap-2">
              ...
              Agrupamento de Clientes ({groups.length} grupo
              {groups.length !== 1 && "s"})
            </div>
          </AccordionTrigger>
